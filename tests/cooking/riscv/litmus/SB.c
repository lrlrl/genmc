...
inline static int final_cond(int _out_0_x7,int _out_1_x7) {
  switch (_out_0_x7) {
  case 0:
    switch (_out_1_x7) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x7=%d; 1:x7=%d;\n",c,show ? '*' : ':',(int)o[out_0_x7_f],(int)o[out_1_x7_f]);
}
...
"#END _litmus_P0\n"
:[x7] "=&r" (_a->out_0_x7[_i])
:[x5] "r" ((int)(1)),[x6] "r" (&_a->x[_i]),[x8] "r" (&_a->y[_i])
:"cc","memory"
);
  }
  mbar();
...
"#END _litmus_P1\n"
:[x7] "=&r" (_a->out_1_x7[_i])
:[x5] "r" ((int)(1)),[x6] "r" (&_a->y[_i]),[x8] "r" (&_a->x[_i])
:"cc","memory"
);
  }
  mbar();
...
static void init(ctx_t *_a) {
  ...
  _a->seed = rand();
  _a->out_0_x7 = malloc_check(size_of_test*sizeof(*(_a->out_0_x7)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      int _out_0_x7_i = ctx.out_0_x7[_i];
      int _out_1_x7_i = ctx.out_1_x7[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_0_x7_i,_out_1_x7_i));
      o[out_0_x7_f] = _out_0_x7_i;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x7=0 /\\ 1:x7=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=1be21f7e9dba92a472a306f5f690a1f6\n");
  fprintf(out,"Cycle=Fre PodWR Fre PodWR\n");
  fprintf(out,"Relax SB %s %s\n",p_true > 0 ? "Ok" : "No","PodWR");
  fprintf(out,"Safe=Fre\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=PodWR Fre PodWR Fre\n");
